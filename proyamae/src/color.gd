...
# **왕조를 24/18 로 내리고 나머지를 그 아래로 다시 깝니다.** 예전 값(왕조 34/26)
# 에서는 팀컬러 하나가 카드 스무 장 몫을 해서, 무엇을 모았느냐보다 무엇을
# 켰느냐가 더 크게 작용했습니다. 순서는 그대로 두고 전체를 눌렀습니다.
#
#   왕조 > 단일연도 > 단일팀 > 듀얼팀 > 시대
const B_DYNASTY := [24, 18]
const B_YEAR := [17, 13]
const B_FRANCHISE := [15, 11]
const B_DUAL := [10, 8]
