...
## 그리기 헬퍼.
##
## 탄이 수백 발 깔리는 장르라 도형은 되도록 싸게 그린다. 엔진이 배칭해 주는
## draw_circle / draw_line 을 우선 쓰고, 실루엣이 필요할 때만 fill_fan 으로 넘긴다.
...
static func ellipse_pts(c: Vector2, rx: float, ry: float, seg: int = 20) -> PackedVector2Array:
	var pts := PackedVector2Array()
	if rx <= 0.0 or ry <= 0.0:
		return pts
	for i in seg:
...
## 바탕 · 그늘 · 외곽선 세 겹.
##
## 그늘은 폴리곤을 x>0 반쪽과 교차시켜 만드는데, 이 계산은 **로컬 좌표에서 한 번만**
## 하면 된다. 매 프레임 돌리면 Geometry2D 가 프레임을 먹는다 — [Art] 가 시작할 때
## 조각을 만들어 두고 그릴 때는 변환만 건다.
static func shade_of(p: PackedVector2Array) -> Array:
	var box := PackedVector2Array([
		Vector2(0, -400), Vector2(400, -400), Vector2(400, 400), Vector2(0, 400)])
	return Geometry2D.intersect_polygons(p, box)
